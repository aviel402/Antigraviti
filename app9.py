...
<div id="pause-screen" class="screen hidden" style="background: rgba(10, 15, 25, 0.9); z-index:9000;">
    ...
    <button class="menu-btn" onclick="location.reload()" style="background:#e74c3c; border-color:#e74c3c;">☒ בחירת דמות (אתחול)</button>
    ...
    upd() {
        ...
        if(this.y+this.h > floor_lvl){
            ...
            currentMap.platforms.forEach(pl_form => {
               ...
               if(this.vy>=0 && this.y+this.h >= pFloorY - 12 && this.y+this.h <= pFloorY + 12 && this.x+this.w>pl_form.x && this.x < pl_form.x+pl_form.w){
...
function loadWv() {
    ...
    aBox.innerText = currentMap.is_boss ? "BOSS BATTLE ENGAGED" : `STAGE ${wave}`;
